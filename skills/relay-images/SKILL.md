---
name: relay-images
description: "Generate and edit raster images through a user-configured OpenAI-compatible relay. Use when the user explicitly asks for relay-based image generation or editing, including 文生图, 图生图, local reference images, multi-image composition, masks, quality or size controls, output formats, compression, or streamed partial images. Do not use for ordinary image requests that should use the built-in image generator, for SVG or other deterministic vector work, or for direct OpenAI Platform billing."
---

# Relay Images

Use the bundled relay client to create or edit images while keeping project files on the local client. The relay only carries model requests and image data.
...
4. Choose the operation:
   ...
   - Any source image, reference image, composition input, or mask: `edit`.
...
10. Use `generate` for text-to-image or `edit` for one or more local images. The stable forms are:
