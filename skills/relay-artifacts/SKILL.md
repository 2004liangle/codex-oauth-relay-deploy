...
4. 每次只选择一种操作：
   ...
   - 需要真正透明的抠图结果：图片任务添加 `--background transparent --format png`。默认模型带细发丝 Alpha 精修，优先用于人物和商品；纯平涂动漫在默认模型丢轮廓时再尝试 `--cutout-model isnet-anime`。服务端会在上传前验证 Alpha 通道，不合格的结果不会标记为完成。
...
默认模型优先保留人物发丝；纯平涂动漫在默认模型丢失外轮廓时再添加 `--cutout-model isnet-anime`。不要只在提示词中要求透明，也不要把棋盘格当作透明通道。
