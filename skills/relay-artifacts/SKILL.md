...
description: 通过已配置的 OpenAI 兼容异步中转服务生成、编辑或抠出图片主体，并借助飞书云盘中转、下载图片和附件。适用于支持 Agent Skills 的客户端中的文生图、图生图、多图合成、抠图、去背景、透明 PNG、附件中转和弱网下载，尤其适合本地项目文件必须留在客户端、Base64 图片回传或大文件直传容易失败的情况。
---
...
3. 选择一种传输方式：
   - **客户端工具，推荐：** 用客户端已登录的飞书云盘工具上传本地文件，再把本地路径和上传后返回的文件 token 传给 `manifest`，由它计算文件名、MIME 类型、字节数和 SHA-256。把生成的 JSON 交给 `submit-edit`、`cutout` 或 `submit-handoff`。任务完成后，用 `download` 获取输出清单，再用同一个云盘工具下载清单中的文件 token。
   - **脚本直传，备用：** 直接把本地路径传给 `edit` 或 `handoff`。这种方式需要私密的飞书应用凭据，由脚本自行完成上传和下载。
4. 根据任务阶段选择操作：
   - 没有原图：使用 `generate` 文生图。
   - 有一张或多张原图、参考图，或者使用蒙版：使用 `edit`。
   - 已有定稿图，用户只要求“抠图”“去背景”或“做成透明 PNG”：必须使用 `cutout`。服务端固定调用即梦 Agent，并验证真实 Alpha 通道。
   - 用户同时要求生成或精修人物，并交付透明 PNG：先用 `generate`/`edit` 完成人物内容和边缘精修；检查定稿图后，再把这张定稿图交给 `cutout` 一次。不要让即梦承担人物生成、造型修改或内容精修，也不要把多个阶段合并成即梦提示词。
   - 把普通附件交给可信服务端处理：使用 `handoff`。
   - `generate`/`edit` 的 `--background transparent --format png` 仅为旧任务兼容参数，不代表专用抠图，也不能替代 `cutout`。
...
```bash
python3 scripts/relay_artifacts.py manifest \
  --file person.png --file-token HOST_RETURNED_TOKEN --role image \
  > person.json

python3 scripts/relay_artifacts.py cutout \
  --input-manifest @person.json --wait
```

没有客户端云盘工具、已私下配置飞书应用凭据时，可让脚本直传并下载：

```bash
python3 scripts/relay_artifacts.py cutout \
  --image person.png --wait --download-dir output
```

`cutout` 不接收提示词、模型或质量参数。服务端只向即梦 Agent 提交一次固定抠图指令；人物生成、造型修改和边缘精修应在调用它之前完成。不要把棋盘格或白底图当作透明通道。
